...
namespace TeamSuneat.Audio
{
    public partial class AudioManager : SingletonMonoBehaviour<AudioManager>
    {
        ...
        /// <summary>  카메라와 타겟 사이의 최대 거리 </summary>
        private const float MAX_CAMERA_TARGET_DISTANCE = 12;

        private const int INITIAL_AUDIO_OBJEC_CAPACITY = 32;
        ...
        private List<AudioObject> _audioObjects = new(INITIAL_AUDIO_OBJEC_CAPACITY);
        ...
        public AudioObject AmbienceAudioObject => _ambienceAudioObject;

        private AudioObject _bgmAudioObject;
        private AudioObject _ambienceAudioObject;

        private AudioCrossFader _bgmFader;
        private AudioCrossFader _ambienceFader;
        ...
        protected override void Awake()
        {
            ...
            _ambienceFader = new AudioCrossFader(this);
        }
        ...
        public void SetAmbienceMixerVolume()
        {
            if (Mixer == null)
            {
                if (Log.LevelWarning)
            ...
            float volume = GameSetting.Instance.Audio.MusicVolume * GameSetting.Instance.Audio.AmbienceVolume;
            _ = Mixer.SetFloat("Ambience", Mathf.Log10(volume) * 20);
        }
        ...
        private bool CheckPlayableAmbience(AudioClip clip)
        {
            if (!Application.isPlaying)
            {
                return false;
        ...
        public AudioObject PlayAmbience(SoundNames soundName)
        {
            SoundAsset soundAsset = ScriptableDataManager.Instance.FindSound(soundName);
            return !soundAsset.IsValid() || !soundAsset.SoundClips.IsValid()
                ? null
                : PlayAmbience(soundAsset.SoundClips[0], soundAsset.VolumeScale);
        }

        public AudioObject PlayAmbience(AudioClip clip, float volume = 1f)
        {
            if (!CheckPlayableAmbience(clip))
            {
                return null;
            }
            ...
            if (audioObject != null)
            {
                audioObject.Type = AudioTypes.Ambience;
                audioObject.PlayLoopScaled(clip);
                audioObject.SetVolume(volume);
                audioObject.SetMixerGroup(MusicMixerGroup);

                _ambienceAudioObject = audioObject;

                if (Log.LevelInfo)
                {
                    Log.Info(LogTags.Audio, "(Manager) 환경음(Ambience)을 재생합니다. {0}", clip.name);
                }
            }
        ...
        {
            if (_ambienceAudioObject != null)
            {
                Log.Info(LogTags.Audio, "(Manager) 환경음(Ambience) 페이드를 정지합니다. {0}", _ambienceAudioObject.name);
            }

            _ambienceFader.StopFade();
            Release(ref _ambienceAudioObject);
        }

        public bool TryStartChangeAmbience(SoundNames soundName)
        {
            SoundAsset asset = ScriptableDataManager.Instance.FindSound(soundName);
            if (!asset.IsValid() || !asset.SoundClips.IsValid())
            {
            ...
            else if (_ambienceAudioObject == null)
            {
                return PlayAmbience(soundName);
            }
            else if (_ambienceAudioObject.CompareClip(audioClip))
            {
                if (Log.LevelWarning)
                {
                    Log.Warning(LogTags.Audio, "(Manager) 같은 이름의 환경음(Ambience)을 재생하지 않습니다: {0}", soundName);
                }

                return false;
            }

            StartChangeAmbience(audioClip, asset.VolumeScale);
            return true;
        }

        public void StartChangeAmbience(AudioClip audioClip, float volume = 1f)
        {
            AudioObject newAudioObject = Spawn(audioClip.name, transform);
            if (newAudioObject != null)
            {
                ...
                if (Log.LevelInfo)
                {
                    Log.Info(LogTags.Audio, "(Manager) 환경음(Ambience)을 페이드 변경합니다: {0} >> {1}", _ambienceAudioObject.name, audioClip.name);
                }

                _ambienceFader.StartFade(_ambienceAudioObject, newAudioObject, CROSS_FADE_DURATION, result =>
                {
                    _ambienceAudioObject = result;
                });
            }
        }
        ...
        private bool CheckPlayableSFX(SoundAsset asset)
        {
            ...
            if (GameSetting.Instance.Audio.MasterVolume <= 0f || GameSetting.Instance.Audio.SFXVolume <= 0f)
            {
                return false;
            }
        ...
        public AudioObject PlayDonDestoryedSFXOneShotUnscaled(SoundAsset asset, Vector3 position)
        {
        ...
        public AudioObject PlayDonDestoryedSFXOneShotScaled(SoundAsset asset, Vector3 position)
        {
        ...
        private AudioObject PlaySFXOneShot(SoundAsset soundAsset, Vector3 position, AudioTimeType timeType)
        {
            ...
            if (!CheckDistanceToCameraCloseEnough(soundAsset, position))
            {
                return null;
            }
        ...
        private bool CheckDistanceToCameraCloseEnough(SoundAsset soundAsset, Vector3 spawnPosition)
        {
            if (soundAsset.UseCheckDistance)
            {
                if (!spawnPosition.IsZero())
                {
                    if (CameraManager.Instance != null)
                    {
                        Vector2 cameraPosition = (Vector2)CameraManager.Instance.MainCamera.transform.position;
                        float distance = Vector2.Distance(cameraPosition, spawnPosition);
                        if (distance > MAX_CAMERA_TARGET_DISTANCE)
                        {
                            if (Log.LevelWarning)
                            {
                                Log.Warning(LogTags.Audio, "(Manager) 카메라와 거리가 멀어 효과음({0})을 재생하지 않습니다. Distance: {1}", soundAsset.Name, distance);
                            }
