...
namespace TeamSuneat
{
    ...
    {
        public bool TryConvert(Dictionary<string, string> row, out PlayerCharacterData model, IList<string> warnings)
        {
            model = null;
            ...
            {
                warnings?.Add($"Name {name}: Weapon enum 파싱 실패: {weaponStr}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: GrowStat1 파싱 실패: {growStat1Str}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: GrowStat2 파싱 실패: {growStat2Str}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: BaseStat1 파싱 실패: {baseStat1Str}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: BaseStat2 파싱 실패: {baseStat2Str}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: BaseStat3 파싱 실패: {baseStat3Str}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: GrowStat1Value 파싱 실패: {growStat1ValueStr}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: GrowStat2Value 파싱 실패: {growStat2ValueStr}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: BaseStat1Value 파싱 실패: {baseStat1ValueStr}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: BaseStat2Value 파싱 실패: {baseStat2ValueStr}");
                return false;
            }
            ...
            {
                warnings?.Add($"Name {name}: BaseStat3Value 파싱 실패: {baseStat3ValueStr}");
                return false;
            }
